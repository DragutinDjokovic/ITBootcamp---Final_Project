 ...
 * package rs.itbootcamp.humanity.page.objects;
 * 
 * //apache POI import java.io.File; import java.io.FileInputStream; import
 * java.io.IOException; import org.apache.poi.hssf.usermodel.HSSFRow; import
 ...
 * 
 * //driver.get(HumanityHome.URL); driver.manage().timeouts().implicitlyWait(2,
 * TimeUnit.SECONDS); driver.manage().window().maximize();
 * 
 * HumanityHome.clickLogIn(driver); HumanityHome.setUserName(driver,
 * "jesidata@web-inc.net"); HumanityHome.setPassword(driver, "sifrica");
 * HumanityHome.setSendLogIn(driver);
 * 
 * HumanityStaff.clickStaff(driver); HumanityStaff.clickAddEmployee(driver);
 * 
 * //idemo uvoz excela by ExcelUtils ExcelUtils.setExcel("projekatExcel.xls");
 * ExcelUtils.setWorkSheet(0); int r = ExcelUtils.getRowNumber();
 * 
 * for(int i = 0 ; i < r; i++) { String ime = ExcelUtils.getDataAt(i, 0); String
 * prezime = ExcelUtils.getDataAt(i, 1); String email = ExcelUtils.getDataAt(i,
 * 2);
 * 
 * HumanityStaff.setFirstNameA(driver, ime, i+1);
 * HumanityStaff.setSecondNameA(driver, prezime, i+1);
 * HumanityStaff.setEmailA(driver, email, i+1); } ExcelUtils.closeExcel();
 * 
